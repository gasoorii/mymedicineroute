...
export function Waves({ strokeColor = 'rgba(225,29,72,0.15)', backgroundColor = 'transparent' }: WavesProps) {
  ...
  const setLines = () => {
    ...
    pathsRef.current = []
    const xGap = 8; const yGap = 8
    const oWidth = width + 200; const oHeight = height + 30
  ...
  const movePoints = (time: number) => {
    ...
    linesRef.current.forEach(points => {
      points.forEach(p => {
        ...
        const dx = p.x - mouse.sx; const dy = p.y - mouse.sy
        const d = Math.hypot(dx, dy); const l = Math.max(175, mouse.vs)
        if (d < l) {
          const s = 1 - d / l; const f = Math.cos(d * 0.001) * s
...
const tick = (time: number) => {
  ...
  const dx = mouse.x - mouse.lx; const dy = mouse.y - mouse.ly
