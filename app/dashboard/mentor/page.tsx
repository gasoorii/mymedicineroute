...
type MentorProfile = {
  bio: string; expertise_tags: string[];
  institution: string; role_title: string;
  hours_per_week: string; available: boolean;
};
type MatchRequest = {
  id: string; student_query: string; ai_summary: string;
  status: string; created_at: string; student_id: string;
};
type Conversation = {
  id: string; ai_summary: string; status: string; created_at: string;
};
...
export default function MentorDashboard() {
  ...
  const loadData = async () => {
    ...
    const [{ data: prof }, { data: mentProf }, { data: reqs }, { data: convs }] = await Promise.all([
      ...
      supabase.from('match_requests').select('*').eq('matched_mentor_id', user.id).order('created_at', { ascending: false }),
      supabase.from('conversations').select('*').eq('mentor_id', user.id).order('created_at', { ascending: false }),
    ]);
    ...
    setEditForm({
      bio: mentProf?.bio || '',
      hours_per_week: mentProf?.hours_per_week || '',
      available: mentProf?.available ?? true,
    });
    setLoading(false);
  };
          ...
          <div style={s.content}>
            ...
            {mentorProfile?.expertise_tags?.length > 0 && (
              <div style={s.section}>
                ...
                <div style={s.tagRow}>
                  {mentorProfile.expertise_tags.map(tag => (
            ...
            {pendingRequests.length === 0 && (
              <div style={s.section}>
                <div style={s.emptyState}>
                  ...
                  <p style={s.emptyDesc}>
                    When a student's needs match your expertise, a request will appear here.
                    Make sure your profile is complete and you're set to available.
                  </p>
                      ...
                      {matchRequests.filter(r => r.status !== 'pending').map(req => (
                        <div key={req.id} style={s.listItem}>
                          <div style={s.listItemLeft}>
                            <span style={{
                              ...s.statusDot,
                              background: req.status === 'matched' ? '#22c55e' : '#475569',
                            }} />
                            <div>
                  ...
                  <div key={conv.id} style={{ ...s.listItem, cursor: 'pointer' }}>
                    <div style={s.listItemLeft}>
                      ...
                      <div>
                        ...
                        {conv.ai_summary && (
                          <div style={s.aiSummaryChip}>Topic: {conv.ai_summary}</div>
                        )}
                        <div style={s.listItemMeta}>
              ...
              <div style={s.profileCardHeader}>
                <div>
                  ...
                  <div style={s.profileMeta}>
                    {mentorProfile?.role_title} · {mentorProfile?.institution}
                  </div>
                </div>
                <button onClick={() => setEditMode(!editMode)} style={s.editBtn}>
                ...
                <div style={{ display: 'grid', gap: '20px', marginTop: '24px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <button
                      ...
                      style={{
                        ...s.toggleBtn,
                        background: editForm.available ? '#22c55e' : '#334155',
                      }}
                    >
...
function RequestCard({ req, onAccept, onDecline }: {
  req: MatchRequest;
  onAccept: (id: string, studentId: string) => void;
  ...
  return (
    <div style={s.requestCard}>
      ...
      <div style={s.requestActions}>
        <button onClick={() => onAccept(req.id, req.student_id)} style={s.acceptBtn}>
          Accept & Connect
        </button>
        <button onClick={() => onDecline(req.id)} style={s.declineBtn}>
          Decline
        </button>
      </div>
    </div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  sidebar: {
    width: '240px', flexShrink: 0, background: '#111213',
  ...
  avatar: {
    width: '32px', height: '32px', borderRadius: '50%', background: '#E11D48',
  ...
  signOutBtn: {
    ...
    fontWeight: 600, cursor: 'pointer', padding: '8px 12px', textAlign: 'left',
    borderRadius: '8px',
  },
  ...
  requestTag: {
    ...
    border: '1px solid rgba(225,29,72,0.2)', padding: '3px 10px', borderRadius: '999px',
  },
  ...
  acceptBtn: {
    ...
    borderRadius: '999px', padding: '10px 20px', fontSize: '14px', fontWeight: 700,
    cursor: 'pointer', boxShadow: '0 4px 12px rgba(225,29,72,0.25)',
  },
  declineBtn: {
    background: 'none', color: '#64748b', border: '1px solid rgba(255,255,255,0.1)',
  ...
  editBtn: {
    ...
    borderRadius: '999px', padding: '8px 18px', fontSize: '13px', fontWeight: 600, cursor: 'pointer',
  },
  ...
  input: {
    ...
    borderRadius: '12px', fontSize: '15px', color: '#e2e8f0',
  ...
  toggleBtn: {
    border: 'none', borderRadius: '999px', padding: '6px 16px',
