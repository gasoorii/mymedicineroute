...
type StudentProfile = { school: string; grade: string; interest: string };
type Conversation = { id: string; ai_summary: string; status: string; created_at: string; mentor_id: string };
type MatchRequest = { id: string; student_query: string; ai_summary: string; status: string; created_at: string };
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      // Realtime — match_requests
      matchSub = supabase
        .channel('student-match-requests')
        .on('postgres_changes', {
          event: '*',
          schema: 'public',
  ...
  const loadData = async (userId: string) => {
    const [{ data: prof }, { data: stuProf }, { data: convs }, { data: reqs }] = await Promise.all([
      ...
      supabase.from('student_profiles').select('school, grade, interest').eq('id', userId).single(),
      supabase.from('conversations').select('*').eq('student_id', userId).order('created_at', { ascending: false }),
      supabase.from('match_requests').select('*').eq('student_id', userId).order('created_at', { ascending: false }),
    ]);
    setProfile(prof);
          ...
          <div style={s.content}>
            ...
            <div style={s.statRow}>
              {[
                ...
                <div key={stat.label} style={s.statCard}>
                  <div style={{ ...s.statValue, ...(stat.small ? s.statValueSmall : {}) }}>{stat.value}</div>
            ...
            <div style={s.section}>
              <div style={s.sectionHeader}>
                ...
                <button onClick={() => setTab('match')} style={s.sectionAction}>+ New request</button>
              </div>
              {matchRequests.length === 0 ? (
                <EmptyState icon="⟡" title="No requests yet"
                  desc="Submit a question or research challenge to get matched with a mentor."
                  action="Find a Mentor" onAction={() => setTab('match')} />
              ) : (
                <div style={s.list}>
                  {matchRequests.slice(0, 3).map(req => (
                    <div key={req.id} style={s.listItem}>
                      <div style={s.listItemLeft}>
                        <span style={{ ...s.statusDot, background: req.status === 'matched' ? '#22c55e' : req.status === 'pending' ? '#f59e0b' : '#94a3b8' }} />
                        <div>
                          <div style={s.listItemTitle}>{req.student_query.slice(0, 80)}{req.student_query.length > 80 ? '...' : ''}</div>
        ...
        {tab === 'match' && (
          <div style={s.content}>
            <div style={s.pageHeader}>
              ...
              <p style={s.pageSub}>Describe your research question or what you're struggling with. Our AI will summarize your needs and match you with the right mentor.</p>
            </div>
            {matchSuccess ? (
              <div style={s.successCard}>
                <div style={s.successIcon}>✓</div>
                <h3 style={s.successTitle}>Request submitted</h3>
                <p style={s.successText}>Our AI is reviewing your question and finding the best mentor match. You'll be notified when a mentor is connected.</p>
                <button onClick={() => setMatchSuccess(false)} style={s.btnRed}>Submit another request</button>
              </div>
            ) : (
              <div style={s.matchCard}>
                <div style={s.matchCardHeader}>
                  ...
                  <p style={s.matchCardSub}>Be specific. The more detail you give, the better the match.</p>
                </div>
                <form onSubmit={submitMatchRequest}>
                  <textarea value={query} onChange={e => { setQuery(e.target.value); setMatchError(''); }}
                    rows={6} style={s.textarea}
                    placeholder="e.g. I want to study the correlation between social media use and sleep quality in Bahraini high school students..." />
                  <div style={s.charCount}>{query.length} / 1000 characters</div>
                  {matchError && <p style={s.errorText}>{matchError}</p>}
                  <button type="submit" disabled={submitting || query.trim().length < 20}
                    style={{ ...s.btnRed, width: '100%', opacity: submitting || query.trim().length < 20 ? 0.5 : 1, cursor: submitting || query.trim().length < 20 ? 'not-allowed' : 'pointer' }}>
                    {submitting ? 'Matching you with a mentor...' : 'Submit for Matching →'}
                  </button>
                </form>
                    ...
                    <div key={req.id} style={s.listItem}>
                      <div style={s.listItemLeft}>
                        ...
                        <div>
                          ...
                          {req.ai_summary && <div style={s.aiSummaryChip}>AI summary: {req.ai_summary}</div>}
                          <div style={s.listItemMeta}>{req.status.charAt(0).toUpperCase() + req.status.slice(1)} · {new Date(req.created_at).toLocaleDateString()}</div>
                        </div>
                      </div>
                    </div>
              ...
              <div style={s.list}>
                {conversations.map(conv => (
                  <div key={conv.id} style={{ ...s.listItem, cursor: 'pointer' }}
                    onClick={() => router.push(`/dashboard/student/chat/${conv.id}`)}>
                    <div style={s.listItemLeft}>
                      <div style={s.convAvatar}>M</div>
                      <div>
                        <div style={s.listItemTitle}>Mentor Chat</div>
                        {conv.ai_summary && <div style={s.listItemMeta}>Topic: {conv.ai_summary}</div>}
...
const s: Record<string, React.CSSProperties> = {
  ...
  matchCard: { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '32px', marginBottom: '32px' },
  matchCardHeader: { marginBottom: '24px' },
  aiTag: { display: 'inline-block', fontSize: '11px', fontWeight: 700, letterSpacing: '2px', textTransform: 'uppercase', color: '#E11D48', background: 'rgba(225,29,72,0.1)', border: '1px solid rgba(225,29,72,0.2)', padding: '4px 10px', borderRadius: '999px', marginBottom: '16px' },
  ...
  textarea: { width: '100%', padding: '16px', border: '1.5px solid rgba(255,255,255,0.1)', borderRadius: '14px', fontSize: '15px', color: '#e2e8f0', background: 'rgba(255,255,255,0.04)', outline: 'none', resize: 'vertical', fontFamily: "'Inter', sans-serif", lineHeight: 1.6, boxSizing: 'border-box' },
  ...
  howItWorks: { marginTop: '28px', paddingTop: '24px', borderTop: '1px solid rgba(255,255,255,0.06)' },
  ...
  howNum: { width: '22px', height: '22px', borderRadius: '50%', background: 'rgba(225,29,72,0.15)', color: '#E11D48', fontSize: '11px', fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  howText: { fontSize: '13px', color: '#64748b', lineHeight: 1.5, paddingTop: '3px' },
  successCard: { background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '20px', padding: '48px 32px', textAlign: 'center', marginBottom: '32px' },
  successIcon: { width: '48px', height: '48px', borderRadius: '50%', background: 'rgba(34,197,94,0.15)', color: '#22c55e', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 800, margin: '0 auto 20px' },
